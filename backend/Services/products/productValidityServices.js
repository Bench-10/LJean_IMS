...
export const getProductValidity = async(branchId) =>{
    const { rows } =  await SQLquery(`
            SELECT ${correctDateFormat('date_added')}, ${correctDateFormat('product_validity')}, Inventory_product.product_name, branch_id, Category.category_name, quantity_added, (product_validity - CURRENT_DATE) <= 3 AS near_expy, product_validity <= CURRENT_DATE AS expy
            FROM Add_Stocks
