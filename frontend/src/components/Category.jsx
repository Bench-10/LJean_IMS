    ...
    <div >
      ...
      <dialog className='bg-transparent fixed top-10 lg:top-0 bottom-0 z-[9999]' open={isCategoryOpen}>
        ...
        <div className="relative flex flex-col border border-gray-600/40 bg-white h-[75vh] lg:h-[600px] w-[95vw] max-w-[600px] rounded-xl p-4 lg:p-6 pb-14 border-gray-300 animate-popup mx-auto my-auto overflow-hidden">
          ...
          {/*CATEGORIES TITLE*/}
          <div className='flex text-center mt-2 lg:mt-4'>
            <h1 className='w-full text-xl sm:text-3xl lg:text-4xl font-bold' >
              CATEGORIES
            </h1>
                ...
                {listCategories.map((row, rowIndex) => (
                  <tr key={rowIndex} className='h-12 sm:h-14 hover:bg-gray-100 transition-colors'>
                    ...
                    <td className='text-center text-sm px-2 sm:px-4'>
                      <button className='bg-blue-700 hover:bg-blue-600 px-3 sm:px-5 py-1 border rounded-lg text-white text-xs sm:text-sm' onClick={() => { setOpenEdit(true); setSelectEditCategory(row) }}>Edit</button>
                    </td>
                  </tr>
          ...
          {openEdit && (
            //OVERLAY
            <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-[9999]" onClick={() => {
              setOpenEdit(false);
              setSelectEditCategory({});
              setEditcategoryName('');
            }}>

              <div className="relative w-[90vw] max-w-[400px] bg-white rounded-lg p-4 sm:p-6 shadow-lg" onClick={(e) => e.stopPropagation()}>
                <button
                  type="button"
