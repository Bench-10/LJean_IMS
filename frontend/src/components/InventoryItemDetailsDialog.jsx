...
function InventoryItemDetailsDialog({ open, onClose, user, item }) {
  ...
  return (
    <div>
      {open && (
        <div
          className="fixed inset-0 bg-black/35 bg-opacity-50 z-20 backdrop-blur-[1px]"
          style={{ pointerEvents: 'auto' }}
          onClick={handleOverlayClick}
        />
      )}

      <dialog className="bg-transparent fixed top-0 bottom-0 z-30" open={open} onClick={handleOverlayClick}>
        <div
          className="relative flex flex-col border border-gray-600/40 bg-white w-[900px] rounded-md py-5 px-3 animate-popup"
          ...
          <div className="pb-4 pt-2 px-8 w-full flex-1 flex flex-col">
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              ...
              <div>
                ...
                <div className="p-2 bg-gray-50 border rounded text-sm">{item.category_name}</div>
              </div>
              <div>
                <label className="text-xs font-bold text-gray-600">UNIT</label>
                <div className="p-2 bg-gray-50 border rounded text-sm">{item.unit}</div>
              </div>
              <div>
                <label className="text-xs font-bold text-gray-600">UNIT PRICE</label>
                <div className="p-2 bg-gray-50 border rounded text-sm text-right">{currencyFormat(item.unit_price)}</div>
              </div>
              {isBranchManager && (
                <div>
                  <label className="text-xs font-bold text-gray-600">UNIT COST</label>
                  <div className="p-2 bg-gray-50 border rounded text-sm text-right">{currencyFormat(item.unit_cost)}</div>
                </div>
              )}
              <div>
