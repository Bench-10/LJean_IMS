...
function NavBar({ setOpenNotif, unreadCount, onOpenRequestMonitor }) {
  ...
  const handleRequestClick = () => {
    if (typeof onOpenRequestMonitor === 'function') {
      ...
      // No-op fallback to avoid hiding the icon entirely
      console.warn('[NavBar] onOpenRequestMonitor not provided.');
    }
  };
  ...
  return (
    <>
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 z-50 bg-navBackground p-4 flex items-center justify-between">
        ...
        {/* Right: Request Status, Notifications, Menu */}
        <div className="flex items-center gap-2">
          ...
          {canSeeNotifications && typeof setOpenNotif === 'function' && (
            <button
              type="button"
              className="relative p-2 border-2 rounded-lg transition-all cursor-pointer border-white text-white hover:text-green-400 hover:border-green-400 hover:bg-green-400 hover:bg-opacity-10"
      ...
      {/* Sidebar / Nav */}
      <nav
        className={`fixed top-0 left-0 h-screen w-[var(--sidebar-width)] bg-navBackground text-white p-3 box-border transition-transform duration-300 ease-in-out flex flex-col ${
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden">
          <ul className="flex flex-col gap-2 [&>a]:py-2 [&>a]:px-3 [&>a]:rounded-md [&>a]:transition-all [&>a]:border-l-4-transparent [&>a]:cursor-pointer [&>a:hover]:bg-[#254717] [&>a]:flex [&>a]:items-center [&>a]:gap-x-[7px]">
            ...
            {roles.some(r => ['Branch Manager','Owner','Inventory Staff'].includes(r)) && (
              <NavLink
                to="/inventory"
                className={({ isActive }) => (isActive ? "border-l-8 bg-[#254717] border-l-green-400" : "")}
            ...
            {roles.some(r => ['Branch Manager','Owner'].includes(r)) && (
              <NavLink
                to="/dashboard"
                className={({ isActive }) => ((isActive || currentLocaion) ? "border-l-8 bg-[#254717] border-l-green-400" : "")}
                onClick={() => setShowMobileMenu(false)}
              >
