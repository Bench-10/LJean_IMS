...
function NavBar({ setOpenNotif = () => {}, unreadCount = 0, onOpenRequestMonitor }) {
  ...
  // Derive current path without useLocation()
  const pathname =
  ...
  const canSeeRequestStatus = roles.some(r =>
    ['Inventory Staff', 'Branch Manager'].includes(r)
  );
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden min-h-0 pb-24">
          <ul className="flex flex-col text-[15px] gap-2
                         [&>a]:py-2 [&>a]:px-3 [&>a]:rounded-md [&>a]:transition-all
                         [&>a]:border-l-4-transparent [&>a]:cursor-pointer
                         [&>a:hover]:bg-[#254717] [&>a]:flex [&>a]:items-center [&>a]:gap-x-[7px]">

            {roles.some(r => ['Branch Manager', 'Owner'].includes(r)) && (
              <NavLink
                to="/dashboard"
                className={({ isActive }) =>
                  (isActive || inBranchArea)
                    ? "text-[15px] border-l-8 bg-[#254717] border-l-green-400"
                    : ""
                }
