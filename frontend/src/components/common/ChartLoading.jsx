  ...
  const content = (
    <div className="flex flex-col items-center justify-center h-full w-full bg-white rounded-md py-6">
      ...
      <div className={`flex ${type === 'chart' ? 'space-x-2' : 'space-x-1'} mt-2`}>
        <div className={`${type === 'chart' ? 'w-3 h-3' : 'w-2 h-2'} bg-green-600 rounded-full animate-bounce animation-delay-0`}></div>
        <div className={`${type === 'chart' ? 'w-3 h-3' : 'w-2 h-2'} bg-green-600 rounded-full animate-bounce animation-delay-150`}></div>
        <div className={`${type === 'chart' ? 'w-3 h-3' : 'w-2 h-2'} bg-green-600 rounded-full animate-bounce animation-delay-300`}></div>
      </div>
