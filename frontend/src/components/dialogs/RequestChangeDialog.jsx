  ...
  return (
    <div className="fixed inset-0 z-[350] flex items-center justify-center bg-black/40" onClick={() => (loading ? null : onCancel && onCancel())}>
      <div className="relative w-[min(680px,95%)] rounded-2xl bg-white p-6" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mt-4">
          <label className="text-xs text-gray-600">Comment <span className="text-red-600">*</span></label>
          <textarea
            value={comment}
            ...
            aria-required="true"
          />
          {((touched || attemptedSubmit) && comment.trim().length === 0) && (
            <div className="text-sm text-red-600 mt-2">Comment is required to request changes.</div>
          )}
        </div>
        <div className="mt-4 flex justify-end gap-2">
          <button className="rounded-md border px-4 py-2 text-sm" onClick={onCancel} disabled={loading}>Cancel</button>
          <button
            className={`rounded-md px-4 py-2 text-sm text-white ${comment.trim().length === 0 ? 'bg-gray-400 cursor-not-allowed' : 'bg-emerald-600 hover:bg-emerald-700'}`}
            onClick={() => {
